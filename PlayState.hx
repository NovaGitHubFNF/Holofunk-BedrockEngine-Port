...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		beWatermark = new FlxText(0, FlxG.height - 70, 0, "Bedrock Engine: v" + MainMenuState.bedrockEngineVersion, 16);
		...
		peWatermark = new FlxText(0, FlxG.height - 50, 0, "Psych Engine: v" + MainMenuState.psychEngineVersion, 16);
		...
		// And this is for HoloFunk

		// LEAVING THIS IN BECAUSE I HATE MYSELF
		//hfWatermark = new FlxText(0, FlxG.height - 30, 0, "HoloFunk: v5.0.0 (Aloe)", 16);
		//hfWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		//hfWatermark.scrollFactor.set();
		//hfWatermark.updateHitbox();
		//hfWatermark.x = FlxG.width - hfWatermark.width - 5;
		//hfWatermark.visible = false;
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode /*keyPressByController*/))
		{
	...
	function spawnNoteSplashOnNote(note:Note, opponent:Bool = true)
	{
